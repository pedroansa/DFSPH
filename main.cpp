...
public:
      ...
      const int res_x, const int res_y, const int f_width, const int f_height)
  {
    ...
    for (int j = 0; j < _resY; ++j) {
        for (int i = 0; i < _resX; ++i) {
            if (i == 0 || j == 0 || i == _resX - 1 || j == _resY - 1) {
    ...
    int n = 15;
    //sample a fluid mass
    for (int j = 8; j < f_height; ++j) {
        for (int i = 8; i < f_width+8; ++i) {
            _pos.push_back(Vec2f(i + 0.25, j + 0.25));
            _pos.push_back(Vec2f(i + 0.75, j + 0.25));
            _pos.push_back(Vec2f(i + 0.25, j + 0.75));
            _pos.push_back(Vec2f(i + 0.75, j + 0.75));
    ...
    _velS = std::vector<Vec2f>(_pos.size(), Vec2f(0, 0));
    ...
    _n = std::vector<std::vector<int>>(_pos.size(), std::vector<int>());
  ...
  void update()
  {
    ...
    applyBodyForce();
    applyViscousForce();
    Vec2f vMax = Vec2f(0, 0);
    for (int i = 0; i < _vel.size(); i++) {
        if (vMax < _vel[i]) {
            vMax = _vel[i];
        }
    }
    _CFL = 0.4 * _h / vMax.length();
    _dt = fmin(_dt, 0.5 * _CFL);

    // predict velocities vi*
    updateVelocityS();

    // correct density error
    correctDensityError(); // fulfill p* - p0 = 0
    updatePosition();
    resolveCollision();

    //update
    buildNeighbor();
    //compute densities and presures
    computeDensity();
    //compute factor alpha
    computeAlpha();
    correctDivergenceError(); //fulfill Dp/Dt = 0
    updateVelocity();
    //resolveCollision();
...
private:
    ...
    // We'll compute the factors alphas
    void computeAlpha() {
        Vec2f aux;
        Real par1;
        Real par2;
        ...
        for (tIndex i = 0; i < _pos.size(); i++) {
           ...
           for (auto& particle : _n[i]) {
               if (particle != i) {
                   aux += _m0 * _kernel.grad_w(_pos[i] - _pos[particle]);
                   par2 += (_m0 * _kernel.grad_w(_pos[i] - _pos[particle])).lengthSquare();
                }

           }    
            par1 = aux.lengthSquare();
            par1 = fmax(par1, 0);
            par2 = fmax(par2, 0);
            _alpha[i] = _d[i] / (par1 + par2);
        }
    }
    ...
    void buildNeighbor()
    {
        ...
        for (int i = 0; i < particleCount(); i++) {
            ...
            for (auto& point : neighbors) {
                for (int p : _pidxInGrid[idx1d(std::get<0>(point), std::get<1>(point))]) {
                    if (i == p)
                        continue;
                    _n[i].push_back(p);
                }
    ...
    void correctDivergenceError() {
        ...
        std::vector<Real> _Dp = std::vector<Real>(_pos.size(), 0);
       
        while (iter < 1 || dp > 0.001) {
            if (iter > 5) {
                break;
            }
            ...
            for (int i = 0; i < _pos.size(); i++) {
                ...
                for (auto& p : _n[i]) {
                    if (p != i) {
                        auto aux1 = _kernel.grad_w(_pos[i] - _pos[p]);
                        auto aux2 = (_velS[i] - _velS[p]);
                        gradV += _m0 * (_velS[i] - _velS[p]).dotProduct(_kernel.grad_w(_pos[i] - _pos[p])); // idk if this abs should be here
                    }
                }
                _Dp[i] = fmax(0,gradV); 
            }
            ...
            for (int i = 0; i < _pos.size(); i++) {
                acumV = Vec2f(0,0);
                ki = fmax(_Dp[i] * _alpha[i] / _dt, 0);

                for (auto& p : _n[i]) {
                    if (i == p)
                        continue;
                    kj = fmax(_Dp[p] * _alpha[p] / _dt, 0);
                    auto aux = ((ki / _d[i]) + (kj / _d[p]));
                    acumV += _m0 * ((ki / _d[i]) + (kj / _d[p])) * _kernel.grad_w(_pos[i] - _pos[p]);
                }
                if (_bo[i] == true)
                    _velS[i] = Vec2f(0, 0);
                else
    ...
    void correctDensityError() {
        ...
        int iter = 0;
        float pavg = mean(_d);
        std::vector<Real>  ps = std::vector<Real>(_d.size()); // vector of estimated densities
        Real ki, kj;
        // Auxiliar var
        Real sum;
        Vec2f sumV;

        while ((pavg - _d0) > n || iter < 2) { 
            if (iter > 20)
                break;
            // First compute the estimated density for every particle
            for (int i = 0; i < ps.size(); i++) {
                ps[i] = 0.0f;
                sum = 0;

                for (auto& p : _n[i]) {
                    sum += _m0 * (_velS[i] - _velS[p]).dotProduct(_kernel.grad_w(_pos[i] - _pos[p])); 
                }
                sum = fmax(sum, 0);
                ps[i] = _d[i] + (_dt * sum);
            }
            // Now we have to update our velocitites based on ki kj and alpha
            for (int i = 0; i < ps.size(); i++) {
                ki = (ps[i] - _d0) * _alpha[i]/_dt/_dt; 
                ki = fmax(0, ki);
                sumV = Vec2f(0,0);
                for (auto& p : _n[i]) {
                    kj = (ps[p] - _d0) * _alpha[p]/_dt/_dt;
                    kj = fmax(0, kj);
                    sumV += _m0 * ((ki / _d[i]) + (kj / _d[p])) * _kernel.grad_w(_pos[i] - _pos[p]);
                }
                // It won't update if it's a boundary
                if(_bo[i] == false)
                    _velS[i] -= _dt * sumV;
                else {
                    _velS[i] = Vec2f(0,0);
            ...
            // We use the mean to see if the variance is not zero
            pavg = mean(ps);
            iter++;
        }
    }
...
SphSolver gSolver(0.08, 0.5, 1e3, Vec2f(0, -59.8), 0.01, 7.0);
